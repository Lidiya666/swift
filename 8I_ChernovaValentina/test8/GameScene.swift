...
class GameScene: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        let line = Line(position: CGPoint(x: view.scene!.frame.minX, y: view.scene!.frame.minY+105), width: Int(view.scene!.frame.maxX))
        self.addChild(line)
    ...
    func createApple(){
        ...
        let randY  = CGFloat(arc4random_uniform(UInt32(view!.scene!.frame.maxY-110)) + 1)
        // Создаем яблоко
        let apple = Apple(position: CGPoint(x: randX, y: randY+104))
...
extension GameScene: SKPhysicsContactDelegate {
    ...
    func didBegin(_ contact: SKPhysicsContact) {
        ...
        switch collisionObject {
            ...
            /*case CollisionCategories.Snake: //не поулчается врезаться в тело, потому что она и так врезается в саму себя при создании
                let newScene = GameScene(size: self.size)
                newScene.scaleMode = self.scaleMode
                let animation = SKTransition.fade(withDuration: 3.0)
                self.view?.presentScene(newScene, transition: animation)*/
